...
We have  ```r max(scraped_reviews_clean$review_id) ``` unique reviews, with the first review issued at ```r min(scraped_reviews_clean$date_posted)``` and the newest review, as of time of scraping, posted on ```r max(scraped_reviews_clean$date_posted)```
